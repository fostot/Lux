...
namespace Lux.UI.Widgets
{
    ...
    public static class TextUtil
    {
        ...
        /// <summary>
        /// Truncate text to fit within a maximum pixel width using MonoFont (8px per char).
        /// Adds "..." if truncated. Pure math — no GPU or atlas needed.
        /// </summary>
        public static string TruncateMono(string text, int maxPixelWidth)
        {
            if (string.IsNullOrEmpty(text)) return text ?? "";
            if (maxPixelWidth <= 0) return "";
            int maxChars = maxPixelWidth / MonoFont.GlyphWidth;
            if (text.Length <= maxChars) return text;
            if (maxChars <= 3) return text.Substring(0, Math.Max(1, maxChars));

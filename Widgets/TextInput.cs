...
namespace Lux.UI.Widgets
{
    ...
    public class TextInput
    {
        ...
        public string Draw(int x, int y, int width, int height = 28)
        {
            ...
            // X clear button FIRST (before focus handler, which would consume the click)
            const int ClearBtnWidth = 20;
            bool clearClicked = false;
            if (!string.IsNullOrEmpty(_text))
            {
                int clearX = x + width - ClearBtnWidth;
                bool clearHover = WidgetInput.IsMouseOver(clearX, y, ClearBtnWidth, height);

                UIRenderer.DrawTextSmall("X", clearX + 6, y + (height - 11) / 2,
                    clearHover ? UIColors.Error : UIColors.TextDim);
            ...
            if (hasText) textAreaWidth -= ClearBtnWidth + 4; // reserve space for X clear button

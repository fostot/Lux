...
namespace Lux.UI.Widgets
{
    ...
    public static class Checkbox
    {
        ...
        public static bool DrawWithLabel(int x, int y, int width, int height,
            string label, bool isChecked, bool partial = false, bool useMonoFont = false)
        {
            ...
            {
                string displayLabel = TextUtil.TruncateMono(label, labelMaxWidth);
                int textY = y + (height - MonoFont.GlyphHeight) / 2;
                var c = UIColors.Text;
                MonoFont.DrawText(displayLabel, labelX, textY, c.R, c.G, c.B, c.A);
            }
            else
            {
